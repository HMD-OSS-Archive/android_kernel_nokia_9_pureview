...
#define FIH_PROC_TP_FP_EN  "AllHWList/tp_fp_en"
#define FIH_PROC_TP_LONG_PRESS  "AllHWList/tp_long_press"
#define FIH_PROC_TP_LONG_PRESS_TIME  "AllHWList/tp_long_press_time"
#define FIH_PROC_TP_LONG_PRESS_SIZE  "AllHWList/tp_long_press_size"
...
struct fih_touch_cb touch_cb = {
    ...
    .touch_long_press_write = NULL,
    .touch_long_press_read = NULL,
    .touch_long_press_time_write = NULL,
    .touch_long_press_time_read = NULL,
    .touch_long_press_size_write = NULL,
    .touch_long_press_size_read = NULL,
    .touch_fp_en_write = NULL,
    .touch_fp_en_read = NULL,
};
...
static int fih_touch_long_press_read_show(struct seq_file *m, void *v)
{
    if (touch_cb.touch_long_press_read != NULL)
    {
        pr_info("F@Touch Read long press \n");
        seq_printf(m, "%u\n", touch_cb.touch_long_press_read());
    }
    return 0;
}

static int fih_touch_long_press_proc_open(struct inode *inode, struct file *file)
{
    return single_open(file, fih_touch_long_press_read_show, NULL);
};

static ssize_t fih_touch_long_press_proc_write(struct file *file, const char __user *buffer,
    size_t count, loff_t *ppos)
{
    char *buf;
    unsigned int res, long_press = 0;

    if (touch_cb.touch_long_press_write == NULL)
        return -EINVAL;
    ...
    if (copy_from_user(buf, buffer, count))
        return -EFAULT;
    long_press = simple_strtoul(buf, NULL, 10);

    if((long_press != 0) && (long_press !=1))
    {
        pr_err("F@Touch %s, wrong value, long_press = %d, *buf = %x\n", __func__, long_press, *buf);
        return -EINVAL;
    }
    if (touch_cb.touch_long_press_write != NULL)
    {
        pr_info("F@Touch Set long_press\n");
        touch_cb.touch_long_press_write(long_press);
    }
...
static struct file_operations touch_long_press_proc_file_ops = {
    .owner   = THIS_MODULE,
    .write   = fih_touch_long_press_proc_write,
    .open    = fih_touch_long_press_proc_open,
    .read    = seq_read,
    .llseek  = seq_lseek,
    .release = single_release
};

static int fih_touch_fp_en_read_show(struct seq_file *m, void *v)
{
    if (touch_cb.touch_fp_en_read != NULL)
    {
        pr_info("F@Touch Read fp_en \n");
        seq_printf(m, "%u\n", touch_cb.touch_fp_en_read());
    }
    return 0;
}

static int fih_touch_fp_en_proc_open(struct inode *inode, struct file *file)
{
    return single_open(file, fih_touch_fp_en_read_show, NULL);
};

static ssize_t fih_touch_fp_en_proc_write(struct file *file, const char __user *buffer,
    size_t count, loff_t *ppos)
{
    char *buf;
    unsigned int res, fp_en = 0;

    if (touch_cb.touch_fp_en_write == NULL)
        return -EINVAL;
    ...
    if (copy_from_user(buf, buffer, count))
        return -EFAULT;
    fp_en = simple_strtoul(buf, NULL, 10);

    if((fp_en != 0) && (fp_en !=1))
    {
        pr_err("F@Touch %s, wrong value, fp_en = %d, *buf = %x\n", __func__, fp_en, *buf);
        return -EINVAL;
    }
    if (touch_cb.touch_fp_en_write != NULL)
    {
        pr_info("F@Touch Set fp_en\n");
        touch_cb.touch_fp_en_write(fp_en);
    }
...
static struct file_operations touch_fp_en_proc_file_ops = {
    .owner   = THIS_MODULE,
    .write   = fih_touch_fp_en_proc_write,
    .open    = fih_touch_fp_en_proc_open,
    .read    = seq_read,
    .llseek  = seq_lseek,
    .release = single_release
};

static int fih_touch_long_press_time_read_show(struct seq_file *m, void *v)
{
    if (touch_cb.touch_long_press_time_read != NULL)
    {
        pr_info("F@Touch Read long press \n");
        seq_printf(m, "%u\n", touch_cb.touch_long_press_time_read());
    }
    return 0;
}

static int fih_touch_long_press_time_proc_open(struct inode *inode, struct file *file)
{
    return single_open(file, fih_touch_long_press_time_read_show, NULL);
};

static ssize_t fih_touch_long_press_time_proc_write(struct file *file, const char __user *buffer,
    size_t count, loff_t *ppos)
{
    char *buf;
    unsigned int res, long_press_time = 0;

    if (touch_cb.touch_long_press_time_write == NULL)
        return -EINVAL;
    ...
    if (touch_cb.touch_long_press_time_write != NULL)
    {
        pr_info("F@Touch Set long_press_time\n");
        touch_cb.touch_long_press_time_write(long_press_time);
    }
...
static struct file_operations touch_long_press_time_proc_file_ops = {
    .owner   = THIS_MODULE,
    .write   = fih_touch_long_press_time_proc_write,
    .open    = fih_touch_long_press_time_proc_open,
    .read    = seq_read,
    .llseek  = seq_lseek,
    .release = single_release
};

static int fih_touch_long_press_size_read_show(struct seq_file *m, void *v)
{
    if (touch_cb.touch_long_press_size_read != NULL)
    {
        pr_info("F@Touch Read long press size\n");
        seq_printf(m, "%u\n", touch_cb.touch_long_press_size_read());
    }
    return 0;
}

static int fih_touch_long_press_size_proc_open(struct inode *inode, struct file *file)
{
    return single_open(file, fih_touch_long_press_size_read_show, NULL);
};

static ssize_t fih_touch_long_press_size_proc_write(struct file *file, const char __user *buffer,
    size_t count, loff_t *ppos)
{
    char *buf;
    unsigned int res, long_press_size = 0;

    if (touch_cb.touch_long_press_size_write == NULL)
        return -EINVAL;
    ...
    if (copy_from_user(buf, buffer, count))
        return -EFAULT;
    long_press_size = simple_strtoul(buf, NULL, 10);

    if((long_press_size <= 0) || (long_press_size > 30))
    {
        pr_err("F@Touch %s, wrong value, long_press_size = %d, *buf = %x\n", __func__, long_press_size, *buf);
        return -EINVAL;
    }
    if (touch_cb.touch_long_press_size_write != NULL)
    {
        pr_info("F@Touch Set long_press_size\n");
        touch_cb.touch_long_press_size_write(long_press_size);
    }
...
static struct file_operations touch_long_press_size_proc_file_ops = {
    .owner   = THIS_MODULE,
    .write   = fih_touch_long_press_size_proc_write,
    .open    = fih_touch_long_press_size_proc_open,
    .read    = seq_read,
    .llseek  = seq_lseek,
    .release = single_release
};
...
static int __init fih_touch_init(void)
{
    ...
    if(tp_probe_success)
    {
       ...
       if (proc_create(FIH_PROC_TP_LONG_PRESS, 0, NULL, &touch_long_press_proc_file_ops) == NULL)
        {
            pr_err("fail to create proc/%s\n", FIH_PROC_TP_LONG_PRESS);
            return (1);
        }
        if (proc_create(FIH_PROC_TP_FP_EN, 0, NULL, &touch_fp_en_proc_file_ops) == NULL)
        {
            pr_err("fail to create proc/%s\n", FIH_PROC_TP_FP_EN);
            return (1);
        }
        if (proc_create(FIH_PROC_TP_LONG_PRESS_TIME, 0, NULL, &touch_long_press_time_proc_file_ops) == NULL)
        {
            pr_err("fail to create proc/%s\n", FIH_PROC_TP_LONG_PRESS_TIME);
            return (1);
        }
        if (proc_create(FIH_PROC_TP_LONG_PRESS_SIZE, 0, NULL, &touch_long_press_size_proc_file_ops) == NULL)
        {
            pr_err("fail to create proc/%s\n", FIH_PROC_TP_LONG_PRESS_SIZE);
            return (1);
        }
//SW8-JH-ALT test+[
        //F@Touch ALT test
