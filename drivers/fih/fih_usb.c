...
struct fihusb_dev {
	...
	ssize_t	(*print_name)(struct fihusb_dev *sdev, char *buf);
	ssize_t	(*print_state)(struct fihusb_dev *sdev, char *buf);
};
...
static int fihusb_dev_register(struct fihusb_dev *sdev);
static void fihusb_dev_unregister(struct fihusb_dev *sdev);

static void fihusb_set_state(struct fihusb_dev *sdev, int state);
...
struct fihusb_dev sw_pid;
struct fihusb_dev sw_adb_secure_dis;
struct fihusb_dev sw_root_detection;
...
static ssize_t state_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	struct fihusb_dev *sdev = (struct fihusb_dev *)
		dev_get_drvdata(dev);
...
void fihusb_set_state(struct fihusb_dev *sdev, int state)
{
	char name_buf[120];
	char state_buf[120];
	char *prop_buf;
	...
	if (sdev->state != state) {
		...
		if (prop_buf) {
			...
			printk(KERN_ERR "out of memory in fihusb_set_state\n");
			kobject_uevent(&sdev->dev->kobj, KOBJ_CHANGE);
		}
	}
}
EXPORT_SYMBOL_GPL(fihusb_set_state);
...
static void do_open_adb(void)
{
	...
	fihusb_set_state(&sw_adb_secure_dis, 1);

	while( p_usb_pid_mapping[i].pid_no_adb != 0x0 ){
		if( pid == p_usb_pid_mapping[i].pid_no_adb ) {
			sw_pid.state = -1;
			fihusb_set_state(&sw_pid, p_usb_pid_mapping[i].pid_adb );
	...
	sw_pid.state = -1;
	fihusb_set_state(&sw_pid, p_usb_pid_mapping[0].pid_adb );
...
static void do_close_adb(void)
{
	...
	while( p_usb_pid_mapping[i].pid_adb != 0x0 ){
		if( pid == p_usb_pid_mapping[i].pid_adb ) {
			sw_pid.state = -1;
			fihusb_set_state(&sw_pid, p_usb_pid_mapping[i].pid_no_adb );
	...
	sw_pid.state = -1;
	fihusb_set_state(&sw_pid, p_usb_pid_mapping[0].pid_no_adb );
...
static void do_open_debug_port(void)
{
	...
	switch (pid) {
		...
		case 0xC00C: //ptp,mass_storage,diag,serial_smd,adb
			sw_pid.state = -1;
			fihusb_set_state(&sw_pid, 0xC00F);
			pr_info("%s : change PID to 0xC00F\n", __func__);
			break;
		default:
			sw_pid.state = -1;
			fihusb_set_state(&sw_pid, 0xC00E);
...
static void fih_switch_all_port(void)
{
	...
	{
		sw_pid.state = -1;
		fihusb_set_state(&sw_pid, 0xC000);
...
int fih_usb_common_command_run(struct fih_usb_data *input)
{
	...
	case SC_ROOT_STATUS:
		...
		if ((cmnd_1==fmask_1) && (input->cmnd[2]=='I') && (input->cmnd[3]=='H')) {
			fihusb_set_state(&sw_root_detection, 1);
			msleep(500);
...
int fih_usb_create_sys_fs(void)
{
	...
	if (rc) {
		pr_info("F@BAT-1 %s: dev_attr_fih_usb_root_enable failed\n", __func__);
		return rc;
	}
	rc = device_create_file(fih_usb_dev, &dev_attr_fih_usb_log_enable);
	if (rc) {
		pr_info("F@BAT-1 %s: dev_attr_fih_usb_log_enable failed\n", __func__);
		return rc;
	}
...
static int scsi_adb_switch_init(void)
{
	...
	ret = fihusb_dev_register(&sw_pid);
	if (ret < 0) {
		pr_info("register sw_pid failed!\n");
	}
	sw_pid.state = -1;

	sw_adb_secure_dis.name = "sw_adb_secure_dis";
	ret = fihusb_dev_register(&sw_adb_secure_dis);
	if (ret < 0) {
		pr_info("register sw_adb_secure_dis failed!\n");
	}
	sw_adb_secure_dis.state = -1;

	sw_root_detection.name = "sw_root_detection";
	ret = fihusb_dev_register(&sw_root_detection);
	if (ret < 0) {
		pr_info("register sw_root_detection failed!\n");
	}
...
static int fihusb_dev_register(struct fihusb_dev *sdev)
{
	...
	sdev->dev = device_create(fih_usb_class, NULL,
...
err_create_file_1:
	device_destroy(fih_usb_class, MKDEV(0, sdev->index));
...
static int __init fih_usb_init(void)
{
	...
	pr_info("%s fih_usb_init \n", __func__);

	if (!fih_usb_class) {
		fih_usb_class = class_create(THIS_MODULE, "fih_usb");
		if (IS_ERR(fih_usb_class))
			return PTR_ERR(fih_usb_class);
...
static void __exit fih_usb_class_exit(void)
{
	fihusb_dev_unregister(&sw_pid);
	fihusb_dev_unregister(&sw_adb_secure_dis);
	fihusb_dev_unregister(&sw_root_detection);
	class_destroy(fih_usb_class);
}
