...
#ifdef CONFIG_DRM_DSI_AOD_LGD_EA9151
#include "dsi_aod_ea9151.h"
#endif
/**
	...
	u32 bl_lvl)
{
	...
	if(panel->bl_reg_swap){
		swap_bl= ((bl_lvl&0xff)<<8)|((bl_lvl&0xff00)>>8);
		rc = mipi_dsi_dcs_set_display_brightness(dsi, swap_bl);
	}else{
...
#ifdef CONFIG_DRM_DSI_AOD_LGD_EA9151
	rc = dsi_panel_aod_brightness_trigger(panel,bl_lvl);
	if(rc){
		dsi_panel_display_pixel_on(panel,bl_lvl);
		dsi_panel_show_brightness(bl_lvl);
		//Execute AOD brightness only
		return 0;
	}
#endif
#ifdef CONFIG_DRM_DSI_LGD_EA9151_HBM
	if(dsi_get_hbm_state()!=0){
		new_lvl = dsi_panel_hbm_verify(panel,bl_lvl);
		bl_lvl = new_lvl;
	}
#endif
	switch (bl->type) {
