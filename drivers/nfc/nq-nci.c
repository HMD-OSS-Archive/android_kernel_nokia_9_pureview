...
int nfc_ioctl_power_states(struct file *filp, unsigned long arg)
{
	...
	if (arg == 0) {
		 ...
		 */
		/* FIH modify WPI suggestion and add here to amke sure interrupt is enabled in FW DL mode */
		nqx_enable_irq(nqx_dev);
			...
			const struct i2c_device_id *id)
{
	...
	//Remove the following to enable FW re-download workable by WPI suggestion
	//if (r) {
