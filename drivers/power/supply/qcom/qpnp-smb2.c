...
{
	if (chg->panel_on_usb_icl_ua > 0 && chg->panel_on_ctl && !chg->ambient_display_en) {
...
// 0: Panel ON
// 1: Panel OFF or Panel ON with Screen saver or Ambient display
static ssize_t lcm_status_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct smb2 *chip = dev_get_drvdata(dev);
	struct smb_charger *chg = &chip->chg;

	return scnprintf(buf, PAGE_SIZE, "%d\n", chg->ambient_display_en);
}

static ssize_t lcm_status_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct smb2 *chip = dev_get_drvdata(dev);
	struct smb_charger *chg = &chip->chg;
	...
	if (sscanf(buf, "%d", &intval) != 1 || (intval < 0 || intval > 1)) {
		return -EINVAL;
	}

	chg->ambient_display_en = (bool)intval;
...
static DEVICE_ATTR(lcm_status, S_IRUGO | S_IWUSR | S_IWGRP,
		lcm_status_show, lcm_status_store);
...
#define SW_JEITA_FULL_CHARGE_NONE 0
#define SW_JEITA_FULL_CHARGE_WARM 1
#define SW_JEITA_FULL_CHARGE_COOL 2
#define SW_JEITA_HYSTERESIS 50

static int sw_jeita_full_charge(struct smb_charger *chg)
{
	union power_supply_propval val = {0, };
	int rc = 0, temp = 0;
	const char *effective_client_fv;

	/* return if status is NOT full */
	rc = smblib_get_prop_batt_status(chg, &val);
	if (rc < 0) {
		pr_err("Couldn't get batt status rc=%d\n", rc);
		return SW_JEITA_FULL_CHARGE_NONE;
	}
	if (val.intval != POWER_SUPPLY_STATUS_FULL) {
		return SW_JEITA_FULL_CHARGE_NONE;
	}

	/* return if capacity is 100% */
	rc = smblib_get_prop_batt_capacity(chg, &val);
	if (rc < 0) {
	...
	/* return if JEITA_VOTER is NOT effective */
	if (chg->fv_votable == NULL) {
		pr_err("Couldn't find FV votable rc=%d\n", rc);
		return SW_JEITA_FULL_CHARGE_NONE;
	}
	effective_client_fv = get_effective_client(chg->fv_votable);
	if (strcmp(effective_client_fv, "JEITA_VOTER") != 0) {
		pr_err("full charge (effective_client_fv=%s)\n", effective_client_fv);
		return SW_JEITA_FULL_CHARGE_NONE;
	}

	/* check if temp is warm/cool */
	...
	temp = val.intval;
	pr_err("full charge (temp=%d)\n", temp);

	rc = smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_WARM_TEMP, &val);
	if (rc < 0) {
		pr_err("Couldn't get warn temp rc=%d\n", rc);
		return SW_JEITA_FULL_CHARGE_NONE;
	}
	if (temp > val.intval - SW_JEITA_HYSTERESIS) {
		return SW_JEITA_FULL_CHARGE_WARM;
	}

	rc = smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_COOL_TEMP, &val);
	if (rc < 0) {
		pr_err("Couldn't get cool temp rc=%d\n", rc);
		return SW_JEITA_FULL_CHARGE_NONE;
	}
	if (temp < val.intval + SW_JEITA_HYSTERESIS) {
		return SW_JEITA_FULL_CHARGE_COOL;
	}
...
static int smb2_probe(struct platform_device *pdev)
{
	...
	device_create_file(&pdev->dev, &dev_attr_type_c_status);
	...
	if (chg->panel_on_usb_icl_ua != -EINVAL) {
		...
		device_create_file(&pdev->dev, &dev_attr_lcm_status);
#if !defined(CONFIG_AOD_FEATURE)
...
static int smb2_remove(struct platform_device *pdev)
{
	...
	if (chg->panel_on_usb_icl_ua != -EINVAL) {
		...
		device_remove_file(&pdev->dev, &dev_attr_lcm_status);
#if !defined(CONFIG_AOD_FEATURE)
	...
	device_remove_file(&pdev->dev, &dev_attr_type_c_status);
	power_supply_unregister(chg->batt_psy);
	power_supply_unregister(chg->usb_psy);
	power_supply_unregister(chg->usb_port_psy);
